import logging, json
from flask import g, abort, request
from flask_restful import Resource
from endpoints import api
from extensions import db, auth
from models import UserItem, Item
from tools import error

logger = logging.getLogger(__name__)

class Buy(Resource):

    @auth.login_required
    def post(self):
        try:
            if request.json is None: return abort(400, "No json payload provided.")

            purchase_amount = int(request.json.get('amount', 1))
            item_id = request.json.get('item_id')
            if item_id is None: return abort(400, "An item must be specified to purchase.")

            item = Item.query.filter_by(id=item_id).first()
            if item is None: return abort(400, f"The provided id [{item_id}] does not match an item in the database.")
            
            if item.amount < purchase_amount: return abort(400, f"Not enough items for sale. Requested amount is [{purchase_amount}] yet stock is [{item.amount}].")

            item_price = item.get_current_price()
            purchase_cost = item_price * purchase_amount
            if purchase_cost > g.current_user.balance: abort(400, f"Not enough money for purchase.")

            user_item = UserItem.query.filter_by(item_id=item_id).first()
            if user_item is None:
                user_item = UserItem(
                    user_id = g.current_user.id,
                    item_id = item_id,
                    amount = purchase_amount
                )
                db.session.add(user_item)
            else:
                user_item.amount += purchase_amount

            g.current_user.balance -= purchase_cost
            item.amount -= purchase_amount
            db.session.commit()
            return { "data" : f"Purchased [{purchase_amount}] of [{item.name}] for [{purchase_cost}p]."}, 201

        except Exception as e:
            db.session.rollback()
            logger.error("Something went wrong.")
            logger.exception(e)
            return abort(500)

# class Sell(Resource):

#     @auth.login_required
#     def post(self):
#         try:
#             if request.json is None: return error(400, "No json payload provided.")
#             purchase_amount = int(request.json.get('amount', 1))
#             item_id = request.json.get('item_id')
#             if item_id is None: return error(400, "An item must be specified to purchase.")

#             item = next([item for item in g.current_user.items if item.id == item_id], None)
#             if item is None: abort(400, f"User [{user_id}] doesn't have any [{user_id}] to sell.")
#             return 200

api.add_resource(Buy, '/buy')